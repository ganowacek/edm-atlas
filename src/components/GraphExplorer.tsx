...
const COLLIDE: Record<Kind, number> = { hub: 36, family: 32, sub: 22 };
...
const GraphExplorer = forwardRef<GraphHandle, Props>(function GraphExplorer(
  ...
  const families = genres.filter((g) => !g.parentId);
  const subsByParent = (pid: string) => genres.filter((g) => g.parentId === pid);
  ...
  const desiredNodes = useCallback((): { id: string; genre: Genre | null; kind: Kind }[] => {
    ...
    families.forEach((f) => {
      ...
      if (expanded.has(f.id)) {
        subsByParent(f.id).forEach((s) => out.push({ id: s.id, genre: s, kind: 'sub' }));
      }
    });
    return out;
  }, [families, expanded, genres]);
  ...
  useEffect(() => {
    ...
    const sim = d3
      ...
      .force(
        ...
        d3
          ...
          .id((d) => d.id)
          .distance((l) => ((l.source as GNode).kind === 'hub' ? ring1 : 64))
          .strength((l) => ((l.source as GNode).kind === 'hub' ? 0.08 : 0.5))
      )
      .force(
        'charge',
        d3.forceManyBody<GNode>().strength((d) =>
          d.kind === 'hub' ? -120 : d.kind === 'family' ? -680 : -150
        ).distanceMax(520)
      )
      .force(
        'radial',
      ...
      .force('collide', d3.forceCollide<GNode>().radius((d) => COLLIDE[d.kind]).strength(0.9).iterations(2))
      .velocityDecay(0.42)
      .alphaDecay(0.03);
    ...
    sim.on('tick', () => {
      ...
      e.gNode.selectAll<SVGGElement, GNode>('g.gnode')
        .attr('transform', (d) => {
          d.x = Math.max(R[d.kind] + 6, Math.min(e.W - R[d.kind] - 6, d.x ?? cx));
          d.y = Math.max(R[d.kind] + 6, Math.min(e.H - R[d.kind] - 6, d.y ?? cy));
  ...
  const updateGraph = useCallback((instant = false) => {
    ...
    want.forEach((w) => {
      ...
      const node: GNode = {
        ...
        childCount: w.kind === 'family' ? subsByParent(w.id).length : 0,
        x: sx, y: sy,
      };
      e.nodes.push(node);
